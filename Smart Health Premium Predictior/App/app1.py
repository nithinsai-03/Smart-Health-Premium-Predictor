...
try:
    ...
    scaler_young = joblib.load(scaler_young_path) # This is likely a dict
    scaler_rest = joblib.load(scaler_rest_path)   # This is likely a dict
    st.sidebar.success("✅ Models and scalers loaded successfully!")
except Exception as e:
    st.sidebar.error(f"⚠️ Error loading model/scaler files: {e}")
    st.sidebar.warning("Please make sure the files `model_young.joblib`, `model_rest.joblib`, `scaler_with_cols_young.joblib`, and `scalar_with_cols_rest.joblib` are in the same directory as the script.")
...
if submitted:
    ...
    input_numeric = pd.DataFrame([input_data])
    ...
    try:
        # Choose model and scaler dictionary
        if age <= 35:
            model = model_young
            scaler_dict = scaler_young
        ...
        # --- FIX ---
        # 1. Extract the actual scaler and column list from the dictionary.
        #    We assume the keys are 'scalar' and 'columns'.
        try:
            actual_scaler = scaler_dict['scalar'] # Changed 'scaler' to 'scalar'
            scaler_cols = scaler_dict['columns']
        except KeyError as ke:
            st.error(f"⚠️ Error accessing scaler/columns from .joblib file. Expected keys 'scalar' and 'columns', but got error: {ke}")
            st.error(f"Loaded scaler object is a dict with keys: {list(scaler_dict.keys())}")
            st.stop()
        except TypeError:
             st.error(f"⚠️ Error: The loaded scaler object is not a dictionary as expected. It's a {type(scaler_dict)}")
             st.stop()

        # 2. Reorder the input DataFrame to match the scaler's training columns
        try:
            input_numeric_reordered = input_numeric[scaler_cols]
        except KeyError as ke:
            st.error(f"⚠️ Error: Mismatch between input columns and scaler's expected columns. Missing: {ke}")
            st.error(f"Scaler expected: {scaler_cols}")
            st.error(f"We provided: {list(input_numeric.columns)}")
            st.stop()

        # 3. Scale the reordered data and predict
        input_scaled = actual_scaler.transform(input_numeric_reordered)
